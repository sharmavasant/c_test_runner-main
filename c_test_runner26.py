...
def inject_typedefs_into_src(src_dir):
    """
    Prepend typedef block to every real project .h and .c in src_dir,
    searching RECURSIVELY through all subdirectories.
    Skips: system headers, already-patched files, stub files.
    """
    count = 0
    # Use rglob to reach files in ALL subdirs (source/, drivers/, lwip/ etc.)
    for f in list(src_dir.rglob("*.h")) + list(src_dir.rglob("*.c")):
        if f.name in _SYSTEM_HEADERS:
            continue
        try:
            text = f.read_text(encoding="utf-8", errors="ignore")
        except Exception:
            continue
        if "_CTEST_RUNNER_TYPES_" in text:
            continue
        # Skip stub files — they are empty guards
        if "Auto-stub by C Test Runner" in text:
            continue
        f.write_text(_TYPEDEF_BLOCK + text, encoding="utf-8")
        count += 1
    return count


# Global set of basenames that are real project headers (populated in setup)
_REAL_PROJECT_HEADERS: set = set()


def inject_typedefs_into_src_single(file_path):
    """Prepend typedef block to a single file if not already present."""
    try:
        text = file_path.read_text(encoding="utf-8", errors="ignore")
    except Exception:
        return
    if "_CTEST_RUNNER_TYPES_" in text or "Auto-stub by C Test Runner" in text:
        return
    file_path.write_text(_TYPEDEF_BLOCK + text, encoding="utf-8")


def _make_stub(src_dir, full_inc_path):
    """
    Write an empty stub ONLY at the exact full path the #include specifies.

    Key rule: NEVER write a flat stub in src/ root.
    - <drivers/mcan.h>    -> only create src/drivers/mcan.h
    - <lwip/ip_addr.h>    -> only create src/lwip/ip_addr.h
    - src/ip_addr.h is NEVER touched — it is a real project file

    This means the real flat project headers (ip_addr.h, etc.) can never
    be overwritten by stubs, no matter what.
    """
    basename = Path(full_inc_path).name
    if basename in _SYSTEM_HEADERS:
        return   # never shadow system headers

    guard = re.sub(r'[^A-Z0-9]', '_', basename.upper())
    stub_content = (f"/* Auto-stub by C Test Runner */\n"
                    f"#ifndef {guard}\n#define {guard}\n#endif\n")

    full_dest = src_dir / full_inc_path
    # Only write if this is NOT the flat root — never overwrite real project headers
    if full_dest.parent == src_dir:
        # This is a flat include like "somefile.h" with no subdir.
        # Only stub it if no real project header exists for this name.
        if basename not in _REAL_PROJECT_HEADERS and not full_dest.exists():
            full_dest.write_text(stub_content, encoding="utf-8")
    else:
        # Subdir path like drivers/mcan.h — always safe to create
...
def resolve_missing_includes(src_dir):
    """
    Iteratively scan ALL .h and .c files in src_dir for missing includes
    and stub them out with full subdir path stubs.

    Key fix: for angle-bracket includes like <drivers/mcan.h> we check
    whether the FULL PATH src/drivers/mcan.h exists — NOT just whether
    mcan.h exists somewhere flat in src/.  A flat src/mcan.h is NOT
    sufficient for gcc to resolve <drivers/mcan.h>.
    """
    angle_re  = re.compile(r'#\s*include\s*<([^>]+\.h[^>]*)>')
    quoted_re = re.compile(r'#\s*include\s*"([^"]+\.h[^"]*)"')
    total_stubbed = 0

    for _pass in range(100):
        # For angle-bracket: check full path existence
        # For quoted: check basename existence anywhere in src_dir
        present_basenames = {f.name for f in src_dir.rglob("*.h")}
        new_stubs = {}  # full_inc_path -> True
        ...
        for f in list(src_dir.rglob("*.h")) + list(src_dir.rglob("*.c")):
            ...
            # Angle-bracket: FULL PATH must exist under src_dir
            for m in angle_re.finditer(text):
                full_path = m.group(1).strip()
                full_dest = src_dir / full_path
                if not full_dest.exists() and full_path not in new_stubs:
                    new_stubs[full_path] = True

            # Quoted: basename anywhere is fine
            for m in quoted_re.finditer(text):
                full_path = m.group(1).strip()
                basename  = Path(full_path).name
                if basename not in present_basenames and full_path not in new_stubs:
                    new_stubs[full_path] = True

        if not new_stubs:
            break

        for full_path in new_stubs:
            _make_stub(src_dir, full_path)
...
def generate_test_c(fn_info, test_cases, mock_header_stems,
    ...
    BASE_TYPEDEFS = """/* === C Test Runner: base type definitions === */
...
"""
    lines = [
        ...
        "/* No project headers — forward declaration + inlined typedefs */",
        BASE_TYPEDEFS,
    ]
...
# ─── Ceedling helpers ─────────────────────────────────────────────────────────
# Canonical stdint types — written as a real file into src/ so Ceedling
# finds it on the include path before any system header.
# Using a physical file is the only approach that works reliably with
# Ceedling on MinGW/MSYS2 regardless of flag handling.
_STDINT_H_CONTENT = """#ifndef _CTEST_STDINT_H_
#define _CTEST_STDINT_H_
/* Fixed-width integer types for C Test Runner build */
typedef signed   char       int8_t;
typedef signed   short      int16_t;
typedef signed   int        int32_t;
typedef signed   long long  int64_t;
...
#  endif
#endif
#endif /* _CTEST_STDINT_H_ */
"""

_STDBOOL_H_CONTENT = """#ifndef _CTEST_STDBOOL_H_
#define _CTEST_STDBOOL_H_
#ifndef bool
#  define bool  _Bool
#  define true  1
#  define false 0
...
def extract_function(source_file, fn_name):
    """
    Extract just the target function from source_file.
    Returns a minimal .c string containing:
      - All #include lines from the original file
      - Only the target function's body (nothing else)
    This avoids compiling other functions that use unavailable types.
    """
    try:
    ...
    # Build minimal .c with only includes + target function
...
def setup_ceedling_project(project_folder, fn_info, test_cases):
    ...
    # Copy entire project tree into src/ preserving relative paths
    # .c files get main() stripped; .h files are copied as-is
    # System-named headers (math.h etc.) are skipped to avoid shadowing
        ...
        for fname in files:
            ...
            if fname.endswith(".c"):
                ...
                if fname in _SYSTEM_HEADERS:
                    continue  # never shadow system headers
                shutil.copy2(src_file, dest_file)
                # Record relative path so stubs never overwrite real files
                rel_h = str(rel_root / fname).replace("\\", "/")
                _REAL_PROJECT_HEADERS.add(fname)         # basename
                _REAL_PROJECT_HEADERS.add(rel_h)         # relative path
    ...
    extracted = extract_function(fn_info["file"], fn_name)
    ...
    if extracted:
        ...
        # Also inject typedefs into it
        inject_typedefs_into_src_single(extracted_dest)
    else:
        # Fallback: use the full source file
...
def run_with_ceedling(project_folder, fn_info, test_cases):
    ...
    # Log every stub created so user can verify paths
    stub_list = sorted(str(f.relative_to(build_dir))
                       for f in (build_dir / "src").rglob("*.h")
                       if "Auto-stub" in f.read_text(encoding="utf-8", errors="ignore"))
    if stub_list:
        yield "INFO", "Stubs created:\n" + "\n".join(f"  {s}" for s in stub_list) + "\n\n"
    if mock_headers:
