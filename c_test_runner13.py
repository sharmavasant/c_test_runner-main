...
# Typedef block prepended to every .h and .c in src/ (except stdint/stdbool)
...
# Files we must never touch — they define or wrap the real system types
_SKIP_INJECT = {"stdint.h", "stdbool.h", "stddef.h", "string.h",
                "stdio.h", "stdlib.h", "unity.h", "unity_internals.h"}


def inject_typedefs_into_src(src_dir):
    """
    Prepend the typedef block to every .h and .c file in src_dir,
    EXCEPT stdint.h, stdbool.h and other system wrappers.
    Uses #ifndef _CTEST_RUNNER_TYPES_ so the block is harmless even
    if a file is included multiple times.
    """
    count = 0
    for f in list(src_dir.glob("*.h")) + list(src_dir.glob("*.c")):
        if f.name in _SKIP_INJECT:
            continue
        try:
...
def _make_stub(src_dir, name):
    """Write a guard-only stub header with typedef block prepended."""
    if name in _SKIP_INJECT:
        return
...
def generate_test_c(fn_info, test_cases, mock_header_stems, declaring_header=None):
    ...
    # Hardcoded typedefs go directly in the .c file — this is the only
    # approach guaranteed to work before any header is parsed by gcc.
    TYPEDEFS = """typedef signed   char       int8_t;
typedef signed   short      int16_t;
typedef signed   int        int32_t;
typedef signed   long long  int64_t;
typedef unsigned char       uint8_t;
