...
def inject_typedefs_into_src(src_dir):
    """
    Prepend typedef block to every project .h and .c in src_dir.
    Skips system header names and files already patched.
    Also skips stub files (they already have the typedef block).
    """
    count = 0
    for f in list(src_dir.glob("*.h")) + list(src_dir.glob("*.c")):
...
def _make_stub(src_dir, full_inc_path):
    """
    Write a completely empty stub (just include guard, no content at all)
    for a missing SDK header.  We never put typedefs or any content into
    stubs — the stub just needs to exist so the preprocessor does not error.
    Creates both the full subdir path and a flat basename copy.
    """
    basename = Path(full_inc_path).name
    # Never stub real system headers — let gcc find them itself
    if basename in _SYSTEM_HEADERS:
        return
    guard = re.sub(r'[^A-Z0-9]', '_', basename.upper())
    # Completely empty stub — no typedefs, no content, just the guard
    stub_content = (
        f"/* Auto-stub by C Test Runner */\n"
        f"#ifndef {guard}\n#define {guard}\n#endif\n"
    )
    ...
    flat_dest = src_dir / basename
    if not flat_dest.exists():
        flat_dest.write_text(stub_content, encoding="utf-8")
...
"""
    ...
    # Without this, since we have no #include of project headers, Ceedling
    # cannot discover which source file contains the function under test.
    source_basename = Path(fn_info["file"]).name
    lines.append(f'/* Ceedling: tell linker which source file to compile */')
    lines.append(f'TEST_SOURCE_FILE("{source_basename}")')
    lines.append("")
...
def setup_ceedling_project(project_folder, fn_info, test_cases):
    ...
    # Skip any project file whose name collides with a standard C library
    # header (e.g. a project "math.h" that contains SDK typedefs) — those
    # collisions cause gcc to pick up garbage instead of the real system header.
    all_headers = collect_all_headers(project_folder)
    for h in all_headers:
        if h.name in _SYSTEM_HEADERS:
            continue   # never shadow a real system header
