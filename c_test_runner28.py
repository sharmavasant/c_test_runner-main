...
def resolve_missing_includes(src_dir):
    """
    Only stub headers whose basename does NOT exist anywhere in the project.
    If ip_addr.h exists somewhere in src/ (real project file), we NEVER stub
    <lwip/ip_addr.h> — the -I flags in project.yml let gcc find the real file.
    Only truly absent SDK-only headers get empty stubs.
    """
    angle_re = re.compile(r'#\s*include\s*<([^>]+\.h[^>]*)>')
    total_stubbed = 0

    for _pass in range(50):
        # All header basenames present ANYWHERE under src_dir
        ...
        for f in list(src_dir.rglob("*.h")) + list(src_dir.rglob("*.c")):
            ...
            for m in angle_re.finditer(text):
                ...
                if basename in _SYSTEM_HEADERS:
                    continue
                # If the basename exists anywhere in src/, the real file
                # will be found via -I flags — do NOT stub it
                if basename in present_basenames:
                    continue
                full_dest = src_dir / full_path
                if not full_dest.exists():
                    new_stubs.add(full_path)
        ...
        for full_path in new_stubs:
            ...
            stub = (f"/* Auto-stub by C Test Runner */\n"
                    f"#ifndef {guard}\n#define {guard}\n#endif\n")
            full_dest = src_dir / full_path
            full_dest.parent.mkdir(parents=True, exist_ok=True)
            full_dest.write_text(stub, encoding="utf-8")
