...
def _make_stub(src_dir, name):
    """Write a single guard-only stub header."""
...
def resolve_missing_includes(src_dir):
    """
    Iteratively find and stub out every missing angle-bracket include.

    The chain can be deep:
        utility.h
          -> common_include.h        (project header, copied to src/)
             -> ti_drivers_config.h  (project header, copied to src/)
                -> <drivers/hw_include/cslr_soc.h>   MISSING  -> stub
                   (stub is empty so no further deps)
                -> <ti/devices/...>                  MISSING  -> stub

    We keep scanning until no new missing headers are discovered, so every
    level of a transitive chain is covered regardless of depth.

    Also handles quoted includes whose target is NOT in src/ — those get
    stubbed too (e.g. a project header that was too deep to be found by the
    flat-copy because two files shared the same name).
    """
    angle_re  = re.compile(r'#\s*include\s*<([^>]+)>')
    quoted_re = re.compile(r'#\s*include\s*"([^"]+)"')
    total_stubbed = 0

    for _pass in range(50):           # safety cap — 50 passes is more than enough
        ...
        for f in list(src_dir.glob("*.h")):
            ...
            # Angle-bracket includes — always external/SDK
            ...
            # Quoted includes — stub if not present in src/
        ...
        if not new_stubs:
            break                     # nothing new — fully resolved
...
def generate_test_c(fn_info, test_cases, mock_header_stems, direct_headers=None):
    """
    direct_headers: list of header filenames that directly declare the function
    under test.  Only these are #included in the test file — NOT every project
    header — to avoid pulling in SDK headers that cannot be resolved.
    """
    fn  = fn_info["name"]
    ret = fn_info["ret"]
    ...
    # Include only the headers that directly declare the function under test.
    if direct_headers:
        for hname in direct_headers:
...
def setup_ceedling_project(project_folder, fn_info, test_cases):
    ...
    # ── Decide which headers to #include in the test file ────────────────────
    # Only include the header that sits next to the source file under test
    # (and any headers that directly declare the function).
    # Including ALL headers risks pulling SDK deps through transitive chains.
    source_header = Path(fn_info["file"]).with_suffix(".h")
    direct_headers = []
    if source_header.exists():
        direct_headers = [source_header.name]

    # Write test file
    test_c    = generate_test_c(fn_info, test_cases, mock_header_stems, direct_headers or None)
...
class App(tk.Tk):
    ...
    def view_generated(self):
        ...
        source_header = Path(fn["file"]).with_suffix(".h")
        direct = [source_header.name] if source_header.exists() else None
        code = generate_test_c(fn, tcs, self._mock_stems(), direct)
