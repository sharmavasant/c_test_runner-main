...
# Standard C types that project headers commonly use without including stdint.h
# themselves (they rely on a chain like common_include.h -> stdint.h).
# We inject these directly into every copied .h that needs them so gcc
# never sees an "unknown type name" error regardless of include order.
...
_STDINT_PREAMBLE = """/* injected by C Test Runner — ensures stdint/stdbool types are always visible */
#ifndef _CT_RUNNER_STDINT_INJECTED
#define _CT_RUNNER_STDINT_INJECTED
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>
#include <string.h>
...
_ALREADY_HAS_STDINT = re.compile(
    r'#\s*include\s*[<"]\s*(stdint|stdbool|stddef)\.h\s*[>"]')


def inject_stdint_into_headers(src_dir):
    """
    For every .h in src_dir that:
      - uses uint8_t / uint32_t / bool / etc., AND
      - does NOT already include stdint.h / stdbool.h / stddef.h
    prepend a small preamble that pulls in those standard headers.

    This fixes "unknown type name 'uint8_t'" errors that occur when
    utility.h uses these types but relies on common_include.h -> stdint.h
    being included first — an ordering that Ceedling's preprocessor
    does not guarantee.
    """
    count = 0
    for h in src_dir.glob("*.h"):
        try:
            ...
            continue
        if _ALREADY_HAS_STDINT.search(text):
            continue                  # already self-sufficient
        if not _STDINT_TYPES.search(text):
            continue                  # does not use these types
...
def _make_stub(src_dir, name):
    """Write a single guard-only stub header (includes stdint preamble)."""
...
def generate_test_c(fn_info, test_cases, mock_header_stems, declaring_header=None):
    ...
    lines = [
        ...
        "/* Standard headers — always included so types are available */",
        '#include <stdint.h>',
        '#include <stdbool.h>',
        '#include <stddef.h>',
        '#include <string.h>',
