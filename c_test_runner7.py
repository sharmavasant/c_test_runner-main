...
# Standard fixed-width types injected directly as typedefs.
# Using #include <stdint.h> is NOT reliable — on MinGW/MSYS2 the types
# live in compiler-internal _stdint.h files that require specific
# predefined macros to activate.  Hardcoding the typedefs is the only
# approach that works unconditionally across all gcc/MinGW variants.
_STDINT_TYPES = re.compile(
    r'\b(uint8_t|uint16_t|uint32_t|uint64_t|'
    r'int8_t|int16_t|int32_t|int64_t|'
    r'uintptr_t|intptr_t|size_t|ssize_t|'
    r'ptrdiff_t|bool|true|false)\b')

_STDINT_PREAMBLE = """/* === C Test Runner: portable type injection === */
#ifndef _CT_RUNNER_TYPES_INJECTED
#define _CT_RUNNER_TYPES_INJECTED
/* Signed */
...
/* NULL */
  ...
  #define NULL ((void*)0)
#endif
#endif /* _CT_RUNNER_TYPES_INJECTED */
"""

_ALREADY_HAS_TYPES = re.compile(
    r'typedef\s+unsigned\s+(char|short|int)\s+uint(8|16|32)_t')


def inject_stdint_into_headers(src_dir):
    """
    For every .h in src_dir that uses uint8_t / uint32_t / bool / etc.
    and does NOT already define those typedefs, prepend hardcoded typedefs.

    This is more reliable than #include <stdint.h> on MinGW/MSYS2 where
    the types live in compiler-internal files that need specific macros.
    Headers that already contain typedef definitions are left untouched.
    """
    count = 0
    for h in src_dir.glob("*.h"):
        try:
            ...
            continue
        if _ALREADY_HAS_TYPES.search(text):
            continue          # already defines the types itself
        if not _STDINT_TYPES.search(text):
            continue          # does not use these types — skip
        h.write_text(_STDINT_PREAMBLE + text, encoding="utf-8")
        count += 1
    return count


def _make_stub(src_dir, name):
    """Write a guard-only stub header with type definitions included."""
...
def generate_test_c(fn_info, test_cases, mock_header_stems, declaring_header=None):
    ...
    lines = [
        ...
        # Embed the same typedef preamble used for headers so the test
        # file is also self-sufficient regardless of include order.
        _STDINT_PREAMBLE.rstrip(),
        "",
        '#include "unity.h"',
    ]
...
def setup_ceedling_project(project_folder, fn_info, test_cases):
    ...
    # ── Inject stdint/stdbool preamble into headers that use uint8_t etc. ────
    # Project headers often rely on common_include.h pulling in stdint.h,
    # but when Ceedling compiles them independently that chain may not be
    # present.  We patch each header directly so it is self-sufficient.
    inject_stdint_into_headers(src_dir)
...
def run_with_ceedling(project_folder, fn_info, test_cases):
    ...
    yield "INFO", "stdint/stdbool preamble injected into project headers.\n"
    if mock_headers:
